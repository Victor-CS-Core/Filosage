...
const sourceLogoPath = path.join(root, "src", "app", "icon.svg");
const sourceLogo = await readFile(sourceLogoPath);
const sourceLogoText = sourceLogo.toString("utf8");
const logoDataUri = `data:image/svg+xml;base64,${sourceLogo.toString("base64")}`;
...
function wordmark(x, y, color = palette.navy, size = 46, anchor = "start") {
  return `<text x="${x}" y="${y}" text-anchor="${anchor}" fill="${color}" font-family="Inter, Arial, sans-serif" font-size="${size}" font-weight="700" letter-spacing="-1">Erudoza</text>`;
}

function tagline(x, y, color, size = 20, anchor = "start") {
  return `<text x="${x}" y="${y}" text-anchor="${anchor}" fill="${color}" font-family="Inter, Arial, sans-serif" font-size="${size}" font-weight="500">Your daily dose of understanding.</text>`;
}
...
await save("logo/erudoza-icon.svg", sourceLogoText, 128, 128, "Authoritative source logo copy; use whenever the standalone icon is required.");

await saveSvg("logo/erudoza-horizontal.svg", documentSvg({
  width: 620,
  height: 160,
  title: "Erudoza horizontal logo",
  description: "The original Erudoza icon beside the Erudoza wordmark and tagline on a transparent background.",
...
await saveSvg("logo/erudoza-light-placement.svg", documentSvg({
  width: 720,
  height: 260,
  title: "Erudoza logo on a light surface",
  description: "A safe off-white placement around the unmodified Erudoza icon.",
...
await saveSvg("logo/erudoza-dark-placement.svg", documentSvg({
  width: 720,
  height: 260,
  title: "Erudoza logo on a dark surface",
  description: "A safe navy placement around the unmodified Erudoza icon.",
  body: `<rect width="720" height="260" rx="16" fill="${palette.darkCanvas}"/>${logoImage(64, 54, 152)}${wordmark(244, 126, palette.offWhite)}${tagline(244, 158, palette.darkSecondary, 18)}`,
}), 720, 260, "Approved logo placement for navy and dark surfaces.");

await saveSvg("logo/erudoza-social-avatar.svg", documentSvg({
  width: 1200,
  height: 1200,
  title: "Erudoza social avatar",
  description: "A square navy social avatar with generous clear space around the original Erudoza icon.",
...
await save("logo/browser-icon.svg", sourceLogoText, 128, 128, "Browser icon copy; Next.js uses src/app/icon.svg as the runtime source.");

const heroBackground = (dark, mobile = false) => documentSvg({
  width: mobile ? 900 : 1600,
  height: mobile ? 1200 : 900,
  title: `${dark ? "Dark" : "Light"} Erudoza hero background${mobile ? " for narrow layouts" : ""}`,
...
await saveSvg("illustrations/desktop-product-frame.svg", documentSvg({
  width: 1440,
  height: 980,
  title: "Erudoza desktop learning workspace",
  description: "A browser-based Erudoza interface showing a focused learning path, review prompt, and evidence progress.",
...
await saveSvg("patterns/progress-rings.svg", documentSvg({ width: 640, height: 640, title: "Progress ring motif", description: "Concentric progress arcs in the Erudoza palette.", body: `<g fill="none" stroke-linecap="round" transform="rotate(-90 320 320)"><circle cx="320" cy="320" r="230" stroke="${palette.subtle}" stroke-width="26"/><circle cx="320" cy="320" r="230" stroke="${palette.teal}" stroke-width="26" stroke-dasharray="980 1445"/><circle cx="320" cy="320" r="170" stroke="${palette.blue}" stroke-width="18" stroke-dasharray="620 1068"/><circle cx="320" cy="320" r="112" stroke="${palette.coral}" stroke-width="12" stroke-dasharray="360 704"/></g>` }), 640, 640, "Progress and mastery background motif.");
...
function campaign({ width, height, title, description, dark = false, eyebrow = "ERUDOZA", headline, supporting, motif = true }) {
  ...
  return documentSvg({
    ...
    title,
    description,
    body: `<rect width="${width}" height="${height}" fill="${background}"/>${motif ? nodeNetwork(width, height, dark ? 0.18 : 0.13) : ""}<rect x="${left}" y="${logoTop}" width="${logoSize}" height="${logoSize}" rx="${Math.round(logoSize * 0.2)}" fill="${dark ? palette.navy : palette.white}"/>${logoImage(left, logoTop, logoSize)}<text x="${left + logoSize + 32}" y="${logoTop + logoSize * 0.48}" fill="${heading}" font-family="Inter,Arial,sans-serif" font-size="${Math.max(26, Math.round(height * 0.065))}" font-weight="700">Erudoza</text><text x="${left + logoSize + 32}" y="${logoTop + logoSize * 0.73}" fill="${copy}" font-family="Inter,Arial,sans-serif" font-size="${Math.max(15, Math.round(height * 0.032))}">${eyebrow}</text><text x="${left}" y="${textTop}" fill="${heading}" font-family="Inter,Arial,sans-serif" font-size="${Math.max(42, Math.round(height * 0.12))}" font-weight="700" letter-spacing="-2">${headline}</text><text x="${left}" y="${textTop + Math.round(height * 0.13)}" fill="${copy}" font-family="Inter,Arial,sans-serif" font-size="${Math.max(20, Math.round(height * 0.04))}">${supporting}</text>`,
  });
}

const campaigns = [
  ["banners/main-website-banner.svg", 1920, 600, "Erudoza website banner", "Learn anything. Understand everything.", "Clear explanations, personalized practice, real understanding.", false, "WEBSITE"],
  ["banners/call-to-action-banner.svg", 1600, 500, "Erudoza call to action banner", "Make the next idea click.", "Explore a focused learning path at erudoza.com", true, "START LEARNING"],
  ["banners/about-page-banner.svg", 1600, 700, "Erudoza about page banner", "Understanding changes what you can do.", "Learning designed for clarity, practice, and transfer.", false, "ABOUT"],
  ["banners/blog-header.svg", 1600, 600, "Erudoza blog header", "Ideas worth understanding.", "Notes on learning, reasoning, and useful knowledge.", true, "ERUDOZA NOTES"],
  ["banners/open-graph.svg", 1200, 630, "Erudoza Open Graph image", "Learn anything. Understand everything.", "Your daily dose of understanding.", true, "ERUDOZA.COM"],
  ["banners/social-sharing-fallback.svg", 1200, 630, "Erudoza social sharing image", "Clear explanations. Practice that adapts.", "Build real understanding at erudoza.com", false, "ERUDOZA"],
  ["social/linkedin-company-banner.svg", 1128, 191, "Erudoza LinkedIn company banner", "Your daily dose of understanding.", "Clear explanations · Personalized practice · Real progress", true, "ERUDOZA.COM"],
  ["social/x-profile-header.svg", 1500, 500, "Erudoza X profile header", "Learn anything. Understand everything.", "Your daily dose of understanding.", true, "ERUDOZA.COM"],
  ["social/github-social-preview.svg", 1280, 640, "Erudoza GitHub social preview", "Learning built for understanding.", "The web learning experience at erudoza.com", false, "GITHUB"],
  ["social/product-announcement.svg", 1200, 630, "Erudoza product announcement", "Turn a question into a learning path.", "Explain. Practice. Apply. Track progress.", false, "PRODUCT UPDATE"],
  ["social/launch-announcement.svg", 1200, 630, "Erudoza launch announcement", "Erudoza is ready to explore.", "Learn anything. Understand everything.", true, "NOW ON THE WEB"],
  ["social/newsletter-header.svg", 1200, 400, "Erudoza newsletter header", "A clearer way into complex ideas.", "Your daily dose of understanding.", false, "ERUDOZA NOTES"],
  ["social/press-kit-cover.svg", 1600, 2000, "Erudoza press kit cover", "Erudoza brand and product resources", "Web application · erudoza.com", true, "PRESS KIT"],
];

for (const [relativePath, width, height, title, headline, supporting, dark, eyebrow] of campaigns) {
  await saveSvg(relativePath, campaign({ width, height, title, description: `${title} using the original Erudoza logo and brand palette.`, dark, eyebrow, headline, supporting }), width, height, `${title} for web and external brand use.`);
}
...
for (const [file, title, body] of [
  ...
  await saveSvg(file, documentSvg({ width: 48, height: 48, title, description: `${title} interface symbol in the Erudoza visual system.`, body }), 48, 48, `${title} icon for external brand compositions; product UI continues to use Lucide.`);
}
