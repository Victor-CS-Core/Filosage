...
def run(args: argparse.Namespace) -> dict:
    ...
    source = Path(__file__).with_name("inspect-transition-writers.mjs").read_text(encoding="utf8")
    ...
    try:
        ...
        command = ("cd /app && node -e \"const fs=require('fs'),c=require('crypto'),f=fs.readFileSync(process.argv[1],'utf8');setTimeout(()=>process.exit(74),150000).unref();"
            ...
            f".then(m=>m.runTransitionWriters('{args.sha}'))"
        ...
        if not isinstance(value, dict) or value.get("sourceSha") != args.sha or value.get("operation") != "transition-writer-observation" or value.get("schemaVersion") != 1:
