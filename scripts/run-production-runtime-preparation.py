#!/usr/bin/env python3
"""Reviewed production runtime preparation with pinned private transport."""
...
def run(args: argparse.Namespace) -> dict:
    ...
    source = Path(__file__).with_name("prepare-production-runtime.mjs").read_text(encoding="utf8")
    ...
    try:
        ...
        command = ("cd /app && node -e \"const fs=require('fs'),c=require('crypto'),f=fs.readFileSync(process.argv[1],'utf8');setTimeout(()=>process.exit(74),150000).unref();"
            ...
            f"return m.runProductionRuntime('{args.sha}','{args.mode}',JSON.parse(payload).key)}})"
        ...
        if not isinstance(value, dict) or value.get("sourceSha") != args.sha or value.get("operation") != "production-runtime-preparation" or value.get("schemaVersion") != 1:
