#!/usr/bin/env python3
"""Read-only QA outbound vantage baseline with pinned private transport."""
...
def run(args: argparse.Namespace) -> dict:
    ...
    source = Path(__file__).with_name("qa-vantage-baseline.mjs").read_text(encoding="utf8")
    ...
    try:
        ...
        command = ("cd /app && node -e \"const fs=require('fs'),c=require('crypto'),f=fs.readFileSync(process.argv[1],'utf8');setTimeout(()=>process.exit(74),150000).unref();"
            ...
            f".then(m=>m.qaVantageBaseline('{args.operator_egress_sha256}'))"
        ...
        if not isinstance(value, dict) or value.get("sourceSha") != args.sha or value.get("operation") != "qa-vantage-baseline" or value.get("schemaVersion") != 1:
...
def main():
    ...
    try:
        ...
        print("QA vantage baseline failed closed; provider output suppressed.", file=sys.stderr)
        return 1
    finally:
        signal.alarm(0)
