...
export function planAccess(app, auth, env, runnerIpv4) {
 context(env);const rules=rulesOf(app);
 ...
 if(rules.length!==1 || operator.name!==operatorName || operator.action!=='Allow' || !ipv4(ip) || mask!=='32' || operator.ipAddressRange!==`${ip}/32` || !equal(Object.keys(operator).sort(),['action','ipAddressRange','name'])) throw Error('Expected exact operator-only maintenance rule.');
 ...
 const intent={...base,enabled:true,ownedRuleName:rule.name,ownedRuleSha256:hash(rule),operatorRuleSha256:hash(operator),beforeRulesSha256:hash(rules),beforeAppSha256:hash(app),beforeAuthSha256:hash(auth),createdAt:new Date().toISOString()};
...
export function cleanupRules(rules,intent) {
 ...
 if(owned.length>1 || (owned.length===1 && hash(owned[0])!==intent.ownedRuleSha256))throw Error('Owned rule changed; reconcile manually.');
 return rules.filter(r=>r.name!==intent.ownedRuleName);
}
export function unchangedOutsideRules(before,authBefore,after,authAfter) {
 const project = app => {
  const v=structuredClone({id:app.id,identity:app.identity,environment:app.properties?.managedEnvironmentId,configuration:app.properties?.configuration,template:app.properties?.template});
...
export function changeAccess(mode,state,env,observe,patch) {
 ...
 if(mode==='apply') {
  context(env);
  if(state.intent.runId!==env.GITHUB_RUN_ID||state.intent.runAttempt!==env.GITHUB_RUN_ATTEMPT||hash(before.app)!==state.intent.beforeAppSha256||hash(before.auth)!==state.intent.beforeAuthSha256||hash(state.rule)!==state.intent.ownedRuleSha256)throw Error('Prepared access context changed.');
 ...
 if(!equal(rulesOf(before.app),expected))patch(expected);
 const after=observe();
 if(!equal(rulesOf(after.app),expected)||!unchangedOutsideRules(before.app,before.auth,after.app,after.auth))throw Error('Access readback mismatch; owned intent requires reconciliation.');
