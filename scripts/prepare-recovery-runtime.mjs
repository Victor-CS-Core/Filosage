...
export const RECOVERY_HOST='filosage-recovery-20260912-b.postgres.database.azure.com';
export function preparationOptions(dsn, sslMode, targetHost=RECOVERY_HOST) {
  if(targetHost!==RECOVERY_HOST)throw new Error('Only the exact recovery clone is allowed.');
  ...
  return {...options,host:RECOVERY_HOST,application_name:'filosage-clone-runtime-preparation',options:'-c statement_timeout=15000 -c lock_timeout=3000 -c idle_in_transaction_session_timeout=15000'};
}
const requireTrue=(row)=>{if(!row||Object.values(row).some(v=>v!==true))throw new Error('Runtime acceptance failed.');};
export async function prepareRecoveryRuntime(factory,dsn,sslMode,password,mode,schemaSql='SELECT false AS compatible',targetHost=RECOVERY_HOST) {
  if(!['preflight','apply','verify'].includes(mode)||!/^[a-f0-9]{64}$/.test(password))throw new Error('Invalid preparation context.');
  const options=preparationOptions(dsn,sslMode,targetHost);const admin=factory(options);let runtime;let created=false;let stage='admin-connect';
  try {
    ...
    try {
      preflight=(await admin.query(`/* preparation-preflight */ SELECT
        ...
        EXISTS(SELECT 1 FROM pg_roles WHERE rolname='filosage_recovery_runtime') AS role_exists,
    ...
    if(!preflight.role_exists){
      ...
      try {
        ...
        if((await admin.query("SELECT 1 FROM pg_roles WHERE rolname='filosage_recovery_runtime'")).rowCount)throw new Error('Role appeared concurrently.');
        const sql=(await admin.query("SELECT format('CREATE ROLE filosage_recovery_runtime LOGIN PASSWORD %L NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS CONNECTION LIMIT 3',$1::text) AS sql",[password])).rows[0]?.sql;
        if(typeof sql!=='string'||!sql.startsWith('CREATE ROLE filosage_recovery_runtime '))throw new Error('Invalid role statement.');
        await admin.query(sql);
        await admin.query('GRANT CONNECT ON DATABASE filosage TO filosage_recovery_runtime');
        await admin.query('GRANT USAGE ON SCHEMA public TO filosage_recovery_runtime');
        await admin.query('GRANT SELECT,INSERT,UPDATE,DELETE ON TABLE public.filosage_documents TO filosage_recovery_runtime');
    ...
    try {
      catalog=(await admin.query(`/* runtime-catalog */ SELECT
        r.rolcanlogin AND NOT r.rolsuper AND NOT r.rolcreatedb AND NOT r.rolcreaterole AND NOT r.rolinherit AND NOT r.rolreplication AND NOT r.rolbypassrls AND r.rolconnlimit=3 AS attributes,
        ...
        FROM pg_roles r WHERE r.rolname='filosage_recovery_runtime'`)).rows[0];requireTrue(catalog);
    } finally {await admin.query('ROLLBACK');}
    stage='runtime-login';runtime=factory({...options,user:'filosage_recovery_runtime',password});await runtime.connect();
    requireTrue((await runtime.query("/* runtime-identity */ SELECT current_database()='filosage' AND current_user='filosage_recovery_runtime' AS correct")).rows[0]);
    ...
    return {ok:true,mode,created,capacity:{maxConnections:50,reservedConnections:5,superuserReservedConnections:10,recoveryConnections:3,unreservedConnections:35},role:'filosage_recovery_runtime',connectionLimit:3,catalog,runtimeTlsLoginVerified:true,rolledBackDmlVerified:true,currentSchemaSqlVerified:true,oldRuntimeBindingChanged:false};
...
export async function runRecoveryRuntime(expectedSha,mode,password){
  const context={schemaVersion:1,operation:'recovery-runtime-preparation',sourceSha:expectedSha,targetHost:RECOVERY_HOST,mode,observedAt:new Date().toISOString()};
  if(expectedSha!=='93f60f24afe59b19b6a592f455a09e8e813f1f84'||process.env.SITE_VERSION!==expectedSha)throw new Error('Runtime source mismatch.');
  const require=createRequire(`${process.cwd()}/package.json`),pg=require('pg');
  try{return {...context,...await prepareRecoveryRuntime(o=>new pg.Client(o),process.env.DATABASE_URL,process.env.DATABASE_SSL,password,mode,schemaSql)};}
