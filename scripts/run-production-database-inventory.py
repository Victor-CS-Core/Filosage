#!/usr/bin/env python3
"""Bounded read-only catalog probe; review source and exact target before execution."""
...
def run(args: argparse.Namespace) -> dict:
    ...
    source = Path(__file__).with_name("inspect-runtime-database.mjs").read_text(encoding="utf8")
    ...
    try:
        ...
        command = ("cd /app && node -e \"const fs=require('fs'),c=require('crypto'),f=fs.readFileSync(process.argv[1],'utf8');"
            ...
            f".then(m=>m.runRuntimeDatabaseInventory('{args.sha}'))"
        ...
        if not isinstance(value, dict) or value.get("sourceSha") != args.sha or value.get("operation") != "read-only-production-database-catalog" or value.get("schemaVersion") != 1:
...
def main() -> int:
    ...
    try:
        ...
        print("Production database inventory failed closed; no provider output retained.", file=sys.stderr)
        return 1
    finally:
        signal.alarm(0)
