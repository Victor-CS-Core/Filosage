#!/usr/bin/env python3
"""Read-only transition writer observation with pinned private transport."""
...
def run(args: argparse.Namespace) -> dict:
    ...
    source = Path(__file__).with_name("inspect-recovery-consistency.mjs").read_text(encoding="utf8")
    ...
    try:
        ...
        command = ("cd /app && node -e \"const fs=require('fs'),c=require('crypto'),f=fs.readFileSync(process.argv[1],'utf8');setTimeout(()=>process.exit(74),150000).unref();"
            ...
            f".then(m=>m.runRecoveryConsistency('{args.sha}'))"
        ...
        if not isinstance(value, dict) or value.get("sourceSha") != args.sha or value.get("operation") != "recovery-consistency-observation" or value.get("schemaVersion") != 1:
...
def main():
    ...
    args.revision = "filosagestg-app--green-93f60f24-1"
    args.sha = "93f60f24afe59b19b6a592f455a09e8e813f1f84"
    args.image_digest = "sha256:0c006852322a91d5e2540cfd27ab58e47dad33a3a1f793fd6afc1e2240bbadf2"
    ...
    try:
        ...
        print("Transition observation failed closed; provider output suppressed.", file=sys.stderr)
        return 1
    finally:
        signal.alarm(0)
