...
import { ArrowRight, CalendarDays, CheckCircle2, Clock3, Flame, LoaderCircle, Target, TrendingUp } from "lucide-react";
...
export default function ProgressPage() {
  ...
  if (!user) return <AppShell><div className="center-state"><TrendingUp size={26} /><p className="overline">Your progress</p><h1>Turn completed lessons into a durable learning record.</h1><p>Create a free account to sync progress, review timing, saved courses, and notes across devices.</p><button className="button button-primary" onClick={() => void signInWithGoogle()}>Create a free account</button></div></AppShell>;

  return (
    <AppShell>
      <div className="progress-page">
        <header className="page-header"><div><p className="overline">Learning record</p><h1>Your progress</h1><p>Track what you have learned, what is becoming durable, and the most useful next action.</p></div></header>
        {!loaded ? <div className="dashboard-loading"><span /><span /><span /></div> : (
          <>
            <section className="progress-metrics" aria-label="Learning summary"><article><span><Flame size={21} /></span><div><small>Current streak</small><strong>{currentStreak(lessons)} {currentStreak(lessons) === 1 ? "day" : "days"}</strong><em>Consistency, not perfection</em></div></article><article><span><Clock3 size={21} /></span><div><small>Tracked study time</small><strong>{Math.floor(minutes / 60)}h {minutes % 60}m</strong><em>Completed lessons</em></div></article><article><span><CheckCircle2 size={21} /></span><div><small>Concepts mastered</small><strong>{mastered}</strong><em>{lessons.length} lessons learned</em></div></article><article><span><Target size={21} /></span><div><small>First-try accuracy</small><strong>{accuracy ? `${accuracy}%` : "—"}</strong><em>Across retrieval checks</em></div></article></section>
            <div className="progress-layout">
              ...
              <section className="goal-panel"><div className="panel-heading"><div><Target size={19} /><h2>Weekly goal</h2></div><button onClick={() => update((current) => ({ ...current, weeklyLessonGoal: current.weeklyLessonGoal >= 10 ? 3 : current.weeklyLessonGoal + 1 }))}>Adjust</button></div><strong>{Math.min(weeklyCompleted, state.weeklyLessonGoal)} of {state.weeklyLessonGoal} lessons</strong><div className="goal-track"><span style={{ width: `${Math.min(100, (weeklyCompleted / state.weeklyLessonGoal) * 100)}%` }} /></div><p>{weeklyCompleted >= state.weeklyLessonGoal ? "Goal complete. Choose depth over adding more for its own sake." : `${state.weeklyLessonGoal - weeklyCompleted} focused lesson${state.weeklyLessonGoal - weeklyCompleted === 1 ? "" : "s"} to reach this week’s goal.`}</p></section>
              <section className="mastery-panel"><div className="panel-heading"><div><TrendingUp size={19} /><h2>Course mastery</h2></div></div><div className="mastery-list">{progress.length ? progress.map((course) => { const learned = course.completedLessonIds.length; const total = Math.max(course.totalLessons ?? learned, 1); const percent = Math.round((learned / total) * 100); return <button key={course.courseId} onClick={() => router.push(`/course/${encodeURIComponent(course.topic)}?id=${course.courseId}`)}><span><strong>{course.topic}</strong><small>{learned}/{total} lessons</small></span><i><b style={{ width: `${percent}%` }} /></i><em>{percent}%</em></button>; }) : <div className="dashboard-empty compact"><div><strong>No course progress yet</strong><p>Begin a published course to build your learning record.</p></div><button className="button button-secondary" onClick={() => router.push("/library")}>Explore courses</button></div>}</div></section>
              <section className="next-step-panel"><div className="panel-heading"><div><ArrowRight size={19} /><h2>Suggested next steps</h2></div></div>{progress.slice(0, 3).map((course) => { const href = course.nextLessonId ? `/course/${encodeURIComponent(course.topic)}/lesson/${course.nextLessonId}?id=${course.courseId}` : `/course/${encodeURIComponent(course.topic)}?id=${course.courseId}`; return <button key={course.courseId} onClick={() => router.push(href)}><span><strong>{course.nextLessonId ? "Continue" : "Review"} {course.topic}</strong><small>{course.nextLessonTitle ?? "Course map and mastery"}</small></span><ArrowRight size={16} /></button>; })}{!progress.length && <p>Your next useful action will appear after you start a course.</p>}</section>
            </div>
          </>
        )}
      </div>
