        ...
        {!loaded ? <div className="dashboard-loading"><span /><span /><span /></div> : (
          <>
            ...
            <div className="progress-layout">
              ...
              <section className="next-step-panel"><div className="panel-heading"><div><ArrowRight size={19} /><h2>Suggested next steps</h2></div></div>{progress.slice(0, 3).map((course) => <button key={course.courseId} onClick={() => router.push(`/course/${encodeURIComponent(course.topic)}/lesson/${course.lastLessonId}?id=${course.courseId}`)}><span><strong>Continue {course.topic}</strong><small>{course.lastLessonTitle}</small></span><ArrowRight size={16} /></button>)}{!progress.length && <p>Your next useful action will appear after you start a course.</p>}</section>
            </div>
          </>
        )}
      </div>
