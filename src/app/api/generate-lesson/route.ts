  ...
  try {
    ...
    const lessonContext = [
      ...
      assignedSources.length
        ? "Treat the supplied atomic evidence claims as the hard ceiling for the lesson's factual content. The course outline and surrounding context are design constraints, never evidence. Prefer a shorter lesson over filler or model-knowledge elaboration. Write at least one distinct supported sentence for every assigned source, then return exactly one structured citation for each sourced factual sentence. Every externally verifiable factual assertion anywhere in the lesson must either be clearly hypothetical or be directly and conservatively entailed by an assigned atomic evidence claim and represented by a structured citation. Each citation must identify the single most direct evidenceClaimId that supports it, and its claim must be copied verbatim as the exact complete sentence already present in the declared lesson section. Citations may point to learning_objective, connection, content, key_takeaway, experience, guided_practice, transfer_task, visual, interaction, quiz, or quiz_explanation. Never repeat the same claim for the same source within one section. If the exact sentence appears in multiple fields, cite it once in each distinct section where it appears. When several evidence claims from one source support the same sentence, choose only the most direct one. The section value must identify the field where that exact sentence appears. Keep the lesson to at most eight distinct sourced factual assertions so every one can be mapped. Before returning JSON, silently audit every sentence in the learning objective, connection, content, key takeaways, experience, visuals, interactions, guided-practice prompts, hints and model responses, transfer-task criteria and model responses, quiz questions, options, explanations, and option feedback. If an externally checkable sentence is not directly entailed by one atomic evidence claim, delete it, narrow it to the evidence's exact scope, or rewrite it as an explicit hypothetical learner action such as 'For this exercise, ...'. Do not infer mnemonics, category exclusions, definitions, hierarchy purposes, directional outcomes, diagnostic benefits, numbers, causal claims, rules, or method steps unless an atomic evidence claim explicitly states them. Imperative activity directions are not factual evidence: say 'Place these labels...' rather than claiming where labels should be placed or why that arrangement works. Use only assigned source and evidence-claim IDs, add a short source locator when known, and never quote or reproduce source passages."
