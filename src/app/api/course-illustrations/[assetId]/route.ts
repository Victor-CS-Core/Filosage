...
export async function GET(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    if (!asset || asset.contentType !== "image/webp" || (asset.status && asset.status !== "uploaded")) {
      return Response.json({ error: "Illustration not found." }, { status: 404 });
    }
    ...
    const bytes = asset.storage === "azure-blob"
      ? await readCourseIllustrationObject(assetId)
      : typeof asset.data === "string"
        ? decodeBase64(asset.data)
        : null;
    if (!bytes || bytes.byteLength !== storedBytes) {
      return Response.json({ error: "Illustration unavailable." }, { status: 502 });
    }
    ...
    console.error("Course illustration fetch failed:", error);
    return Response.json(
      { error: "Illustration unavailable." },
