              ...
              {!selectedTicket ? <EmptyState icon={FileText} title="Choose a case" body="Select a queue row to inspect its recorded evidence and bounded actions." /> : (
                <>
                  ...
                  <div className={styles.dossierScroll}>
                    ...
                    {selectedTicket.requestContext && <section aria-label="Submitted page context"><h3><FileText aria-hidden="true" size={16} />Submitted page context</h3><dl className={styles.caseMeta}><div><dt>Path</dt><dd><code>{selectedTicket.requestContext.pathname}</code></dd></div>{selectedTicket.requestContext.pageTitle && <div><dt>Page title</dt><dd>{selectedTicket.requestContext.pageTitle}</dd></div>}{selectedTicket.requestContext.feature && <div><dt>Feature</dt><dd>{selectedTicket.requestContext.feature}</dd></div>}</dl></section>}
