...
export default function LegalDocument({
  ...
  return <AppShell><article className="legal-page">
    <header><p className="overline">{eyebrow}</p><h1>{title}</h1><p>{summary}</p><dl><div><dt>Effective</dt><dd>{effectiveDate}</dd></div><div><dt>Contact</dt><dd><a href={`mailto:${LEGAL_CONTACT}`}>{LEGAL_CONTACT}</a></dd></div></dl></header>
