...
export default function MasteryPath({ progress }: { progress: CourseProgress[] }) {
  ...
  return <section className="mastery-path" aria-labelledby="mastery-path-title">
    <header className="panel-heading"><div><BrainCircuit size={19} /><span><p className="overline">Mastery path</p><h2 id="mastery-path-title">How your understanding is holding up</h2></span></div><span>{lessons.length} concept{lessons.length === 1 ? "" : "s"}</span></header>
    {lessons.length ? <div className="mastery-path-layout">
      <div className="mastery-node-list" aria-label="Concept mastery states">
    ...
    </div> : <div className="portfolio-empty"><BrainCircuit size={23} /><div><strong>Your mastery path starts with evidence.</strong><p>Complete a lesson and its retrieval checks to see which concepts are secure and which need another pass.</p></div></div>}
  </section>;
}
