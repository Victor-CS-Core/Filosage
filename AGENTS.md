...
- Use `filosage_multica_event` for lifecycle synchronization when that Hermes plugin is available. In Cursor, use the authenticated `multica` CLI instead (`.cursor/skills/multica-cli/SKILL.md`) for the same high-signal lifecycle contract. Use stable `sync_key` values and reuse known `issue_id` values. Use the Multica CLI for task hierarchy, assignment, comments, and agent administration. Never duplicate an item because delivery is delayed or uncertain.
- Cursor Cloud Agents authenticate the CLI from the `MULTICA_TOKEN` environment secret. Never print that value, put it on a command line, or commit it.
