...
test("actual course route exposes operation cost and refuses research beyond its approved ceiling", async () => {
  ...
  const key = "actual-course-budget-ceiling";
  let dispatches = 0;
  ...
  try {
    ...
    const unapproved = await POST(request("actual-course-no-approval"));
    assert.equal(unapproved.status, 409);
    assert.equal(await getStoredDocument(`generationOperations/${generationOperationId("local-owner", "actual-course-no-approval")}`), null);
    assert.equal(dispatches, 1);
