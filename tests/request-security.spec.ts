...
test.describe("public mutation request perimeter", () => {
  test("rejects cross-site, wrong-content-type, oversized, and malformed requests", async ({ request }, testInfo) => {
    const callerHeaders = { "X-Real-IP": `playwright-${testInfo.project.name}-${crypto.randomUUID()}` };
    ...
    const wrongContentType = await request.post("/api/waitlist", {
      headers: { ...callerHeaders, "Content-Type": "text/plain" },
      data: "not-json",
    });
