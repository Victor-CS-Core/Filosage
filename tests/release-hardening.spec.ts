...
test("every Playwright spec belongs to exactly one execution lane", async () => {
  ...
  expect(contractSuites).toHaveLength(40);
...
test("mobile projects execute only explicitly owned cross-device behavior", () => {
  ...
  // September 12 release discovery: 328 desktop / 45 mobile Chromium / 52 WebKit.
  // The audited owner-navigation/creation contrast and retry regression adds
  // exactly one explicitly owned case to each existing project inventory.
...
test("manual OpenGrep security verifies the approved executable and never grants upload or login privileges", () => {
  const path = ".github/workflows/static-security.yml";
  expect(existsSync(path)).toBe(true);
  const workflow = existsSync(path) ? read(path) : "";
  expect(workflow).toMatch(/permissions:\s*\n {2}contents: read\s*\n/);
  expect(workflow.match(/^on:\s*\n([\s\S]*?)^permissions:/m)?.[1].trim()).toBe("workflow_dispatch:");
