...
The public landing page extends that world through **Course Proof First**: a real published course gives visitors something concrete to inspect. The hero uses live public metadata and the shared `CourseBanner`. Its primary action is **Explore courses**. Three steps explain browsing, creating a free account, and taking lessons; short practice and FAQ sections answer the remaining questions. Use specific language, rules and open spacing; avoid inflated claims, illustrative dashboards and repeated feature grids.
...
- Public home: a short editorial hero uses roughly two-fifths of the desktop width while a live published course cover and outline use the remaining three-fifths; **Explore courses** remains visible in the first viewport, including an empty or failed preview. Three ordered steps become a vertical list on mobile. Browsing is public; taking a course requires a signed-in registered account. Public information can render while the session resolves, while private routes retain their loading gate.
