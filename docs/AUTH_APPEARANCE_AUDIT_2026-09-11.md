...
The first reviewed patch passed an unfiltered local DOM fixture, but actual upload exposed a second cause: Microsoft's hosted CSS sanitizer dropped ID selectors, element-qualified logo selectors, descendant link selectors and placeholder pseudo-elements. Only its boilerplate correction survived. Successful Graph upload/readback did not prove rendered CSS acceptance, so PR landing was held.

The corrected patch uses plain supported `.ext-background-image` to suppress the illustration in dark mode and plain `.ext-header-logo`/`.ext-banner-logo` for light logo backing. `.ext-boilerplate-text` supplies its own contrasting text/background. Unsupported wrapper, placeholder and descendant overrides are removed. The original brand asset bytes and geometry remain unchanged; social-provider marks are unaffected. Fixed light/dark probe assets are regenerated from this stylesheet. The revised regression simulates the observed sanitizer before testing the retained rules. Eight focused self-contained branding checks pass; actual full viewport/logo rendering remains a coordinator-owned hosted check. No claim is made that the rejected ID overrides changed provider wrappers or that placeholder colors were corrected.
...
The second actual upload retained the plain classes: the coordinator observed a navy canvas with no illustration, light backing behind both logos, and a dark explanatory panel with contrasting light text. It exposed a separate input cascade: the provider's more-specific `.ext-input.ext-text-box` left the field transparent and its fixed `#757575` placeholder lacked contrast. The final supported compound selector supplies a white field and dark typed text in both themes; the unchanged placeholder then exceeds 4.5:1 contrast. The regression reproduces the transparent field before this rule and checks light/dark/focused results. No unsupported placeholder pseudo-element is uploaded. Final provider readback remains coordinator-owned.


Coordinator hosted verification after the final CSS upload on September13 at14:14UTC: actual dark sign-in and signup both show the navy canvas without the light illustration, visible original logos on light backings, contrasting explanatory text, and white email fields with navy typed text and the provider's gray placeholder. Empty signup validation displayed readable light-red error text on a dark-red surface; no email/account was submitted. Graph raw stylesheet readback matched committed CSS exactly. This is actual provider rendering evidence, distinct from local fixtures.

Actual light-mode verification was not completed: browser security policy blocked changing the browser appearance setting. Local light/fixed-theme checks pass, but do not substitute for a real hosted light observation. App-selected appearance versus OS/browser theme remains open and is not solved by this CSS correction. No policy workaround, new auth client, account creation, app rebuild or application deployment was performed for these checks.
