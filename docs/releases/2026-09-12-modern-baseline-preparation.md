...
All creation/binding/removal steps are proposed, not executed. This packet must gain the missing probe/account/recovery evidence and fresh prerequisite readbacks before requesting maintenance approval.
...
Previous private PostgreSQL and synthetic/current Blob recovery mechanics passed. They do not yet prove historical asset-to-document consistency or recovery of a usable learner service within the approved 15-minute data-loss / 60-minute recovery targets. Prepare and verify an isolated usable-service rehearsal; do not claim these limits from the 604-second database-only result. The former clone and live rehearsal container were cleaned up. A fresh rehearsal needs an exact isolated app origin/access/auth arrangement, approved representative learner and second-account privacy access, recovery-only DML binding to a fresh private restore, and isolated Blob versions with matching references. Disable live AI/payment/email/webhook/worker effects. Pin its cost/lifetime/cleanup packet before execution; the already-approved recovery targets do not need reconfirmation.

Remaining execution dependencies are explicit: actual restriction coverage from the two confirmed probe locations; complete session/writer attribution; designated real non-owner learner/account access; isolated service/asset-reference recovery acceptance; and an agreed maintenance interruption bound supported by those rehearsals. These cannot be replaced by general release approval. Preparation keeps the live app available. No maintenance approval is requested until the concrete operation is ready.
