...
- **QA infrastructure retirement completed:** QA compute/domain/certificate, DNS records, dedicated alerts, exclusive identity/access and GitHub variables are removed; QA test billing and OAuth integrations are retired. Production configuration/authentication and the real learner’s8%progress/8-carddeck remain intact. QA data and recovery credentials remain retained pending disposition. The follow-up is stopped. See [retirement evidence](../research/artifacts/release-readiness-20260912/qa-retirement-execution-20260913.json).
