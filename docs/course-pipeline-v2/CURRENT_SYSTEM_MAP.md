...
| Stage state | stage transactions in `firebase-server.ts` (Azure PostgreSQL adapter under leftover name) | State-machine transitions persist on private courses; validation/approval commit readiness against exact fingerprints | state tests and route integration assertions |
...
| Deterministic repair | `POST /api/courses/[courseId]/repair` | Allowlisted path operations, document fingerprints, document-store transaction, full revalidation, audit diff, stale-safe undo | V2 regressions; hosted PostgreSQL concurrency proof pending |
