...
**KNOWN PROMOTION BLOCKER:** the current workflow sets job-wide `EXPECTED_AUTH_MODE=direct-google` and applies it to the isolated-QA check. Completed Phase A and Phase B QA must report `migration-dual`, so `scripts/check-production-health.mjs` will reject the authentication-mode mismatch before staging. Do not dispatch this workflow or improvise a bypass. A separately reviewed workflow change must scope the isolated-QA check to `migration-dual` while keeping every inactive production check at `direct-google`; then this runbook contract and the workflow evidence must be reverified. This Task 11 documentation change does not resolve or claim resolution of that workflow blocker.

- [ ] The final local verification ladder is green for the exact commit.
- [ ] The known staging-workflow authentication-mode blocker is resolved by a separately reviewed commit and its exact-SHA checks pass.
