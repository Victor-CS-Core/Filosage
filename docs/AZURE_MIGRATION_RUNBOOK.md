...
The Azure PostgreSQL document store is the live runtime. Firestore REST remains a non-production fallback inside `firebase-server.ts` until that leftover adapter name is retargeted. `scripts/export-firebase-authored-courses.ts` and the `migrate:azure:*` npm scripts are one-off importers for the completed authored-course migration; they are not a production data path.
