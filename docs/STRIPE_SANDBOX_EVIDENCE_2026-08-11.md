...
| Full Filosage account deletion | Partial | The remote Stripe cleanup was proven. This run did not delete a real signed-in Filosage account and therefore did not independently prove the Firestore purge, deletion-lock race handling, or user-facing 409 recovery paths. Those remain covered only by automated application tests. |
