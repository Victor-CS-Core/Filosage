...
Updated September 6, 2026. Source implementation only; no Azure deployment, traffic change, privilege change, hosted test or QA resource retirement is claimed.
...
Review the resource-specific bootstrap/ownership/RBAC cutover below first. Check effective inherited role assignments, not only this template. Provision protected GitHub environments `azure-staging`, `azure-candidate-review`, and `azure-production-promotion` with required reviewers and restricted branch/identity access. Confirm Victor's approval for the specific operation. The repository cannot prove these provider-side protections exist. All three workflows share one GitHub concurrency key. Azure's CLI traffic operation has no compare-and-swap in this implementation: an exclusive operator deployment lease and no competing Portal/CLI writers are required across the final readback and swap.
