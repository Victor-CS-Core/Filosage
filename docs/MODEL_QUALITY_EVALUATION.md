...
The suite is deliberately opt-in because it creates private evaluation courses and consumes OpenAI quota. It refuses known production hosts unless the operator adds a second explicit production override. Prefer a local or preview environment backed by Azure PostgreSQL, or credential-free local mode.
...
Start the app with its normal server configuration, then use a local-mode owner evaluation token:
